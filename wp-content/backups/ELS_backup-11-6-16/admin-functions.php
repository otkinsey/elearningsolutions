  ...
  if(!$questions){
    ...
    foreach($questions as $item){
      ...
      for($o=1;$o<4;$o++){
        ...
        ?>
        <? echo $a; ?>
        <input meta-id="<? echo $options[$a]->meta_id; ?>"  class="multipleChoiceOption" value="<? echo $options[$a]->meta_value; ?>" placeholder="option <? echo $o; ?>" type="text" id="<? echo $post->ID; ?>" name="question-<? echo $q; ?>-option-<?echo $o;?>">
        <?
        $a++;
      }
      ?>
        <label for="question-<? echo $q; ?>-answer">Answer:</label>
        <input meta-id="<? echo $answers[$r]->meta_id; ?>" id="<? echo $post->ID; ?>" class="multipleChoiceAnswer" type="text" name="question-<? echo $q; ?>-answer" value="<? echo $answers[$r]->meta_value; ?>">
      <?
      $q++;
      $r++;
    }
...
function exam_questions(){
  ...
  <? foreach($questions as $item){ var_dump($item);?>
    ...
    <div class="admin_data item-<? echo $a; ?>" style="position:relative;" meta-data= <? echo $item->meta_id; ?>>
      ...
      <i n="<? echo $a; ?>" post-id="<? echo $post->ID; ?>" id="<? echo $item->meta_id; ?>" class="fa fa-times deleteExamQuestion" onclick="deleteExamQuestion(event)" style="position:absolute;top:10px;right:40px;"></i>
    </div>
  <? $a++; } /*end foreach loop*/ ?>
  </div>
  ...
  <script>
    ...
    function addExamQuestion(event){
      ...
      if(n == 1){
        var newQuestion = '<label class="item-'+itemCount+'" id="label_'+itemCount+'" for="" class="questionLabel">Question '+itemCount+'</label><div class="admin_data item-'+metaId+'" style="position:relative;"><input id="input_'+itemCount+'" class="examQuestion item-'+metaId+' input-'+n+'" name="wpcf[question-'+itemCount+']" type="text"><i n="'+n+'" id="'+n+'" class="fa fa-times deleteExamQuestion" onclick="deleteExamQuestion(event)" style="position:absolute;top:10px;right:40px;" /></div>';
        ...
        var newQuestion = '<div class="admin_data item-'+item+'" style="position:relative;"><label class="item-'+metaId+'" id="label_'+itemCount+'" for="" class="questionLabel">Question '+itemCount+'</label><input id="input_'+itemCount+'" class="examQuestion item-'+metaId+' input-'+n+'" name="wpcf[question-'+itemCount+']" type="text"><i n="'+n+'" id="'+n+'" class="fa fa-times deleteExamQuestion" onclick="deleteExamQuestion(event)" style="position:absolute;top:10px;right:40px;" /></div>';
    ...
    function deleteExamQuestion(event){
      ...
      console.log('nVar is '+nVar+' and metaId is '+metaId);
      $.ajax({
        method:'POST',
        url:'../wp-content/plugins/elearningSolutions/ajax.php',
